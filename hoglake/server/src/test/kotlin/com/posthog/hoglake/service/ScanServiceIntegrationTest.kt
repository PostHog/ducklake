...
class ScanServiceIntegrationTest {
    ...
    private fun seed(): Fixture = jdbi.withHandle<Fixture, Exception> { h ->
        val catalogId = h.createQuery(
            "INSERT INTO hog_catalog (name, data_path) VALUES ('cat', 's3://b/cat') RETURNING catalog_id",
        ...
        val tables = listOf("events" to 1L, "plain" to 2L).associate { (name, tableId) ->
            h.createUpdate(
                "INSERT INTO hog_table (catalog_id, table_id, created_snapshot, next_field_id) VALUES (?, ?, 0, 3)",
    ...
    private fun file(path: String, records: Long, pv: List<String?>? = null) =
