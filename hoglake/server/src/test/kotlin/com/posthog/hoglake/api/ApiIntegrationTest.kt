...
class ApiIntegrationTest {
    ...
    private suspend fun HttpClient.putJson(url: String, body: String): HttpResponse =
        put(url) {
            contentType(ContentType.Application.Json)
            setBody(body)
        }
    ...
    @Test
    fun `healthz and openapi yaml are served`() = api { client ->
    ...
    @Test
    fun `full lifecycle - catalog to commit to changes to offsets`() = api { client ->
        ...
        val (storedLower, storedUpper) = db.jdbi.withHandleUnchecked { h ->
            h.createQuery(
    ...
    @Test
    fun `drop table returns commit result and time travel still sees it`() = api { client ->
    ...
    @Test
    fun `commit with stale read_snapshot conflicts with DDL - 409`() = api { client ->
        ...
        """
        val conflicted = client.postJson(
    ...
    @Test
    fun `commit validation failures - 422 with ApiError body`() = api { client ->
        client.postJson("/v1/catalogs", """{"name": "vld", "data_path": "s3://hog/vld"}""")

        // Unknown table.
        val unknown = client.postJson(
    ...
    @Test
    fun `unknown column type in createTable - 422`() = api { client ->
    ...
    @Test
    fun `malformed request bodies - 400 with ApiError body`() = api { client ->
        // Truncated JSON.
        assertApiError(
    ...
    @Test
    fun `unparseable query and path params - 400`() = api { client ->
    ...
    @Test
    fun `missing resources - 404 with ApiError body`() = api { client ->
    ...
    @Test
    fun `duplicate catalog namespace and table - 409`() = api { client ->
     ...
     */
    private fun seedDdlChange(catalogName: String, tableName: String, kind: String) {
