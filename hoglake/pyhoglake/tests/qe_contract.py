...
* SPEC GAP: runExpiry/runCleanup accept a client-sent `batch` query
  parameter that the spec does not define (client offers `batch=`, spec
  declares no query parameters for either operation).
* BUG (client): object names are interpolated into URL paths without
  percent-encoding of reserved characters, so a name containing '?',
  '/', or '#' rewrites the request URL structure (xfail below).
"""
