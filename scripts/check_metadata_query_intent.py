...
ALLOWED_MEMBER_QUERY_CALLS = [
    ...
    },
    {
        ...
        "function": re.compile(
            r"\bunique_ptr\s*<\s*QueryResult\s*>\s+DuckLakeTransaction::ExecuteRaw\s*\(\s*string\s+query\s*\)\s*\{"
        ),
        "pattern": re.compile(r"\bconnection\s*\.\s*Query\s*\(\s*query\s*\)", re.DOTALL),
        "max_count": 1,
    },
    {
        # Quack metadata uses a throwaway DuckDB connection to (re)attach the in-process catalog.
        "path": "src/metadata_manager/quack_metadata_manager.cpp",
        "function": re.compile(
            r"\bunique_ptr\s*<\s*QueryResult\s*>\s+QuackMetadataManager::AttachMetadata\s*\(\s*const\s+string\s*&\s*attach_query\s*\)\s*\{"
        ),
        "pattern": re.compile(r"\bfresh_conn\s*\.\s*Query\s*\(\s*query\s*\)", re.DOTALL),
        "max_count": 2,
    },
]
