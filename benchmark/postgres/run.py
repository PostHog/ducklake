...
class Benchmark:
    def __init__(self, root: Path, output: Path, variant: str):
        self.root = root
        self.output = output
        self.duckdb = root / "build/release/duckdb"
        self.postgres_extension = Path(
            os.environ.get(
                "POSTGRES_EXTENSION",
    ...
    def psql_value(self, sql: str):
        return run(
            ["psql", "-X", "-v", "ON_ERROR_STOP=1", "-qAt", "-c", sql]
        ).stdout.strip()

    def fixture_row_counts(self, operation: str):
        tables_by_operation = {
            "list_snapshots": ("ducklake_snapshot", "ducklake_snapshot_changes"),
            "current_snapshot": ("ducklake_snapshot",),
            "table_info": ("ducklake_table", "ducklake_data_file", "ducklake_delete_file"),
            "cleanup_candidates": ("ducklake_files_scheduled_for_deletion",),
            "file_discovery": ("ducklake_table", "ducklake_data_file", "ducklake_delete_file"),
            "stats_lookup_micro": ("ducklake_table_stats", "ducklake_table_column_stats"),
            "expire_snapshots_dry_run": ("ducklake_snapshot", "ducklake_snapshot_changes"),
            "expire_snapshots": ("ducklake_snapshot", "ducklake_snapshot_changes"),
        }
        tables = tables_by_operation[operation]
        counts = {}
        for table in tables:
            counts[table] = int(
                self.psql_value(f'SELECT count(*) FROM "{self.schema}"."{table}";')
            )
        return counts
    ...
    def duckdb_sql(self, sql: str):
        statement = (
            ...
            f"(DATA_PATH '/work/data/ducklake-bench', METADATA_SCHEMA '{self.schema}'); "
            f"{sql}"
        )
        return run(
    ...
    def query_for(self, operation: str, snapshots: int, selectivity: float):
        ...
        if operation == "stats_lookup_micro":
            table_filter = "AND stats.table_id = 1" if self.variant == "feature" else ""
            return f"""
                SELECT count(*), bit_xor(hash(table_id, column_id, record_count, next_row_id,
                    file_size_bytes, contains_null, contains_nan, min_value, max_value, extra_stats))
                FROM postgres_query('__ducklake_metadata_dl', $stats_query$
                    SELECT stats.table_id, columns.column_id, stats.record_count, stats.next_row_id,
                           stats.file_size_bytes, columns.contains_null, columns.contains_nan,
                           columns.min_value, columns.max_value, columns.extra_stats
                    FROM \"{self.schema}\".ducklake_table_stats stats
                    LEFT JOIN \"{self.schema}\".ducklake_table_column_stats columns USING (table_id)
    ...
    def measure(self, operation: str, snapshots: int, selectivity: float):
        ...
        fixture_row_counts = self.fixture_row_counts(operation)
        before = resource.getrusage(resource.RUSAGE_CHILDREN)
        started = time.perf_counter()
        result = self.duckdb_sql(query)
        elapsed = time.perf_counter() - started
        after = resource.getrusage(resource.RUSAGE_CHILDREN)
        first_field = next(csv.reader([result]))[0] if result else ""
        result_rows = 1 if operation == "current_snapshot" else int(first_field)
        return {
            "operation": operation,
            "snapshots": snapshots,
            ...
            "result": result,
            "fixture_rows": sum(fixture_row_counts.values()),
...
def main():
    ...
    parser.add_argument("--variant", choices=("v153_base", "feature"), required=True)
    parser.add_argument(
        "--operation",
        choices=(
            "list_snapshots",
    ...
    benchmark = Benchmark(args.root, args.output, args.variant)
    for trial in range(args.trial_offset, args.trial_offset + args.trials):
        schema = f"dlbench_{args.variant}_{args.scale}_{args.operation[:12]}_{trial}"
        ...
        elif args.operation == "stats_lookup_micro":
