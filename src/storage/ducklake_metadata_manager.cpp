...
CREATE TABLE {METADATA_CATALOG}.ducklake_file_column_stats(data_file_id BIGINT, table_id BIGINT, column_id BIGINT, column_size_bytes BIGINT, value_count BIGINT, null_count BIGINT, min_value VARCHAR, max_value VARCHAR, contains_nan BOOLEAN, extra_stats VARCHAR, PRIMARY KEY (data_file_id, column_id));
...
CREATE TABLE {METADATA_CATALOG}.ducklake_table_stats(table_id BIGINT PRIMARY KEY, record_count BIGINT, next_row_id BIGINT, file_size_bytes BIGINT);
...
CREATE TABLE {METADATA_CATALOG}.ducklake_partition_info(partition_id BIGINT PRIMARY KEY, table_id BIGINT, begin_snapshot BIGINT, end_snapshot BIGINT);
CREATE TABLE {METADATA_CATALOG}.ducklake_partition_column(partition_id BIGINT, table_id BIGINT, partition_key_index BIGINT, column_id BIGINT, transform VARCHAR, PRIMARY KEY (partition_id, partition_key_index));
...
CREATE TABLE {METADATA_CATALOG}.ducklake_files_scheduled_for_deletion(data_file_id BIGINT PRIMARY KEY, path VARCHAR, path_is_relative BOOLEAN, schedule_start TIMESTAMPTZ);
CREATE TABLE {METADATA_CATALOG}.ducklake_inlined_data_tables(table_id BIGINT, table_name VARCHAR, schema_version BIGINT, PRIMARY KEY (table_id, schema_version));
CREATE TABLE {METADATA_CATALOG}.ducklake_column_mapping(mapping_id BIGINT PRIMARY KEY, table_id BIGINT, type VARCHAR);
CREATE TABLE {METADATA_CATALOG}.ducklake_name_mapping(mapping_id BIGINT, column_id BIGINT, source_name VARCHAR, target_field_id BIGINT, parent_column BIGINT, is_partition BOOLEAN, PRIMARY KEY (mapping_id, column_id));
...
CREATE TABLE {METADATA_CATALOG}.ducklake_macro_impl(macro_id BIGINT, impl_id BIGINT, dialect VARCHAR, sql VARCHAR, type VARCHAR, PRIMARY KEY (macro_id, impl_id));
...
WHERE data.table_id=%d AND data.begin_snapshot <= {SNAPSHOT_ID} AND (
	...
	auto result = SnapshotQuery(end_snapshot, query);
	if (result->HasError()) {
...
FROM main_results
	...
	auto result =
	    has_inlined_table ? transaction.SnapshotQuery(end_snapshot, query) : SnapshotQuery(end_snapshot, query);
	if (result->HasError()) {
...
unique_ptr<QueryResult> DuckLakeMetadataManager::CurrentQuery(DuckLakeSnapshot snapshot, string &query) {
	return transaction.CurrentQuery(snapshot, query);
}

unique_ptr<QueryResult> DuckLakeMetadataManager::CurrentQuery(string &query) {
	return transaction.CurrentQuery(query);
}
...
DuckLakeMetadataManager::ReadInlinedFileDeletionsForRange(TableIndex table_id, DuckLakeSnapshot start_snapshot,
	...
	auto query = StringUtil::Format("SELECT file_id, row_id, begin_snapshot FROM {METADATA_CATALOG}.%s "
	                                "WHERE begin_snapshot >= %d AND begin_snapshot <= {SNAPSHOT_ID}",
	                                inlined_table_name, start_snapshot.snapshot_id);
	auto query_result = SnapshotQuery(end_snapshot, query);
	if (query_result->HasError()) {
...
WHERE inlined_data.begin_snapshot >= %d AND inlined_data.begin_snapshot <= {SNAPSHOT_ID};)",
	...
	auto result = transaction.SnapshotQuery(end_snapshot, query);
	return result;
}
...
ORDER BY table_id NULLS FIRST;
	...
	auto result = CurrentQuery(start_snapshot, query);
	if (result->HasError()) {
...
DuckLakeMetadataManager::GetFilesDeletedOrDroppedAfterSnapshot(const DuckLakeSnapshot &start_snapshot) const {
	// get all changes made to the system after the snapshot was started
	auto result = transaction.CurrentQuery(start_snapshot, R"(
	SELECT data_file_id
	FROM {METADATA_CATALOG}.ducklake_delete_file
