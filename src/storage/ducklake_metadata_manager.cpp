...
CREATE TABLE {METADATA_CATALOG}.ducklake_file_partition_value(data_file_id BIGINT, table_id BIGINT, partition_key_index BIGINT, partition_value VARCHAR, PRIMARY KEY (data_file_id, partition_key_index));
...
FROM main_results
	...
	auto result = has_inlined_table ? transaction.Query(end_snapshot, query) : Query(end_snapshot, query);
	if (result->HasError()) {
