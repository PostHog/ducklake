...
WHERE table_id = {TABLE_ID})";
	...
	auto result = PassthroughQuery(query);
	for (auto &row : *result) {
...
idx_t DuckLakeMetadataManager::GetNetInlinedRowCount(const string &inlined_table_name, DuckLakeSnapshot snapshot) {
	auto query = GetNetInlinedRowCountSql(inlined_table_name);
	auto result = PassthroughQuery(snapshot, query);
	for (auto &row : *result) {
		return row.GetValue<idx_t>(0);
	}
...
string DuckLakeMetadataManager::GeneratePassthroughFileColumnStatsCTEBody(const CTERequirement &req,
                                                                          TableIndex table_id) {
...
DuckLakeMetadataManager::GenerateCTESectionFromRequirements(const unordered_map<idx_t, CTERequirement> &requirements,
                                                            TableIndex table_id, bool metadata_passthrough) {
	...
	for (const auto &entry : requirements) {
		...
		cte_section += metadata_passthrough ? GeneratePassthroughFileColumnStatsCTEBody(req, table_id)
		                                    : GenerateFileColumnStatsCTEBody(req, table_id);
		cte_section += ")";
	}
...
DuckLakeMetadataManager::GenerateFilterPushdownComponents(const FilterPushdownInfo &filter_info, TableIndex table_id,
	...
	result.cte_section =
	    GenerateCTESectionFromRequirements(filter_result.required_ctes, table_id, metadata_passthrough);
...
vector<DuckLakeFileListEntry> DuckLakeMetadataManager::GetFilesForTable(DuckLakeTableEntry &table,
	...
	if (filter_info && !filter_info->column_filters.empty()) {
		auto components = GenerateFilterPushdownComponents(*filter_info, table_id, true);
...
FROM main_results
	...
	auto result = has_inlined_table ? transaction.Query(end_snapshot, query) : PassthroughQuery(end_snapshot, query);
	if (result->HasError()) {
...
string DuckLakeMetadataManager::WriteNewInlinedData(DuckLakeSnapshot &commit_snapshot,
	...
	for (auto &entry : new_data) {
		...
		if (inlined_table_name.empty()) {
			...
			auto result = PassthroughQuery(commit_snapshot, query);
			for (auto &row : *result) {
...
map<idx_t, set<idx_t>> DuckLakeMetadataManager::ReadInlinedFileDeletions(TableIndex table_id,
	...
	auto query = StringUtil::Format("SELECT file_id, row_id FROM {METADATA_CATALOG}.%s WHERE begin_snapshot <= "
	                                "{SNAPSHOT_ID}",
	                                inlined_table_name);
	auto query_result = PassthroughQuery(snapshot, query);
...
DuckLakeMetadataManager::ReadInlinedFileDeletionsForRange(TableIndex table_id, DuckLakeSnapshot start_snapshot,
	...
	auto query = StringUtil::Format("SELECT file_id, row_id, begin_snapshot FROM {METADATA_CATALOG}.%s "
	                                "WHERE begin_snapshot >= %d AND begin_snapshot <= {SNAPSHOT_ID}",
	                                inlined_table_name, start_snapshot.snapshot_id);
	auto query_result = PassthroughQuery(end_snapshot, query);
	if (query_result->HasError()) {
...
string DuckLakeMetadataManager::GetInlinedDeletionTableName(TableIndex table_id, DuckLakeSnapshot snapshot,
	...
	if (create_if_not_exists) {
		auto create_query = StringUtil::Format(
		    ...
		    table_name);
		auto create_result = PassthroughExecute(snapshot, create_query);
...
string DuckLakeMetadataManager::GetPathForSchema(SchemaIndex schema_id,
	...
	auto result = transaction.Query(StringUtil::Format(R"(SELECT path, path_is_relative
FROM {METADATA_CATALOG}.ducklake_schema
WHERE schema_id = %d;)",
	                                                   schema_id.index));
	for (auto &row : *result) {
...
bool DuckLakeMetadataManager::IsColumnCreatedWithTable(const string &table_name, const string &column_name) {
	auto result = transaction.Query(StringUtil::Format(R"(SELECT TRUE
...
string DuckLakeMetadataManager::GetPathForTable(TableIndex table_id, const vector<DuckLakeTableInfo> &new_tables,
	...
	for (const auto &new_table : new_tables) {
		if (new_table.id == table_id) {
			...
			auto result = transaction.Query(StringUtil::Format(R"(SELECT s.path, s.path_is_relative
...
WHERE schema_id = %d;)",
	...
	auto result = transaction.Query(StringUtil::Format(R"(SELECT
	s.path AS s_path,
...
ORDER BY snapshot_id
	...
	auto res = PassthroughQuery(query);
	if (res->HasError()) {
...
vector<DuckLakeFileForCleanup> DuckLakeMetadataManager::GetOrphanFilesForCleanup(const string &filter,
	...
	auto known_files_res = PassthroughQuery(known_files_query);
...
WHERE data_file_id IN (%s);
	                                ...
	                                deleted_file_ids);
	auto result = PassthroughExecute(query);
	if (result->HasError()) {
...
void DuckLakeMetadataManager::DeleteSnapshotRows(const vector<DuckLakeSnapshotInfo> &snapshots) {
	string snapshot_ids;
...
AND NOT EXISTS (
	...
	result = transaction.Query(StringUtil::Format(R"(SELECT data_file_id, table_id, path, path_is_relative
FROM {METADATA_CATALOG}.ducklake_data_file
...
VALUES %s;
	...
	result = transaction.Query(StringUtil::Format(R"(SELECT delete_file_id, table_id, path, path_is_relative
FROM {METADATA_CATALOG}.ducklake_delete_file
...
vector<DuckLakeTableSizeInfo> DuckLakeMetadataManager::GetTableSizes(DuckLakeSnapshot snapshot) {
	...
	auto result = transaction.Query(snapshot, R"(SELECT
...
void DuckLakeMetadataManager::SetConfigOption(const DuckLakeConfigOption &option) {
	...
	auto result = transaction.Query(StringUtil::Format(R"(SELECT COUNT(*)
