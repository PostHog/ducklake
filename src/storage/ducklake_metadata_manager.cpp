...
WHERE table_id = {TABLE_ID})";
	...
	auto result = Query(query);	for (auto &row : *result) {
...
DuckLakeCatalogInfo DuckLakeMetadataManager::GetCatalogForSnapshot(DuckLakeSnapshot snapshot) {
	...
	auto result = transaction.Query(snapshot, R"(SELECT schema_id, schema_uuid::VARCHAR, schema_name, path, path_is_relative
...
WHERE {SNAPSHOT_ID} >= begin_snapshot AND ({SNAPSHOT_ID} < end_snapshot OR end_snapshot IS NULL)
	...
	result = transaction.Query(snapshot, StringUtil::Format(R"(SELECT schema_id, tbl.table_id, table_uuid::VARCHAR, table_name,
	(
		SELECT %s
...
ORDER BY table_id, parent_column NULLS FIRST, column_order
	...
	result = transaction.Query(snapshot, StringUtil::Format(R"(SELECT view_id, view_uuid, schema_id, view_name, dialect, sql, column_aliases,
	(
		SELECT %s
		FROM {METADATA_CATALOG}.ducklake_tag tag
...
WHERE {SNAPSHOT_ID} >= begin_snapshot AND ({SNAPSHOT_ID} < view.end_snapshot OR view.end_snapshot IS NULL)
	...
	result = transaction.Query(snapshot, StringUtil::Format(R"(SELECT schema_id, ducklake_macro.macro_id, macro_name, (
		SELECT %s
...
WHERE  {SNAPSHOT_ID} >= ducklake_macro.begin_snapshot AND ({SNAPSHOT_ID} < ducklake_macro.end_snapshot OR ducklake_macro.end_snapshot IS NULL)
	...
	result = transaction.Query(snapshot, R"(SELECT partition_id, part.table_id, partition_key_index, column_id, transform
...
ORDER BY part.table_id, partition_id, partition_key_index
	...
	result = transaction.Query(snapshot, R"(SELECT sort.sort_id, sort.table_id, sort_expr.sort_key_index, sort_expr.expression, sort_expr.dialect, sort_expr.sort_direction, sort_expr.null_order
...
auto result = transaction.Query(StringUtil::Format(R"(SELECT mapping_id, table_id, type, column_id, source_name, target_field_id, parent_column, is_partition
...
void DuckLakeMetadataManager::DropEmptySupersededInlinedTables() {
	...
	auto targets = transaction.Query(R"(SELECT idt.table_id, idt.schema_version, idt.table_name
